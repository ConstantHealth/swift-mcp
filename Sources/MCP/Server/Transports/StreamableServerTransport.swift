...
public actor StreamableServerTransport: ServerTransport {
    ...
    public func send(requestId: RequestID, data: Data) async throws {
        ...
        guard let streamID = self.requestToStreamMap[requestId] else {
            throw TransportError.noResponseStreamAvailable
        }
        // and grab the stream itself
        guard let stream = self.streamMap[streamID] else {
            throw TransportError.noResponseStreamAvailable
        }
        ...
        if (try? JSONDecoder().decode(AnyResponse.self, from: data)) != nil {
    ...
    public func handleGet() async throws -> DataStream {
        guard noteStreamPackage == nil else {
            throw MCPError.transportError(TransportError.notificationStreamAlreadyConnected)
        }
